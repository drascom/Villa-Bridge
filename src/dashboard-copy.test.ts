...
test("dashboard widget düzenini hafif ve kalıcı olarak özelleştirir", async () => {
  ...
  assert.match(dashboard, /setInterval\(updateTimeOfDay,60000\)/);
  assert.match(dashboard, /const nightModeActive=hour=>greetingKeyForHour\(hour\)==="greetingNight"\|\|hour<7/);
  assert.match(dashboard, /document\.body\.classList\.toggle\("night",nightModeActive\(new Date\(\)\.getHours\(\)\)\)/);
  assert.match(dashboard, /body\[data-active-view="home"\]::after\{content:"";position:fixed;z-index:8;inset:0;opacity:0;pointer-events:none;background:rgba\(8,17,13,.42\);transition:opacity .9s ease\}/);
  assert.match(dashboard, /body\.night\[data-active-view="home"\]::after\{opacity:1\}/);
  assert.match(dashboard, /:root\[data-theme="dark"\] body\[data-active-view="home"\]::after\{background:rgba\(4,10,8,.22\)\}/);
  assert.match(dashboard, /body\.night\[data-active-view="home"\]\{--home-glass:rgba\(233,239,235,.14\);--home-control:rgba\(238,243,240,.6\);--home-border:rgba\(255,255,255,.3\)\}/);
  assert.match(dashboard, /:root\[data-theme="dark"\] body\.night\[data-active-view="home"\]\{--home-glass:rgba\(12,20,16,.3\)/);
  assert.match(dashboard, /@media\(prefers-reduced-motion:reduce\)\{body\[data-active-view="home"\]::after\{transition:none\}\}/);
  ...
  assert.match(dashboard, /id="createGroupShortcut" class="quiet"/);
  assert.match(dashboard, /\$\("#createGroupShortcut"\)\.setAttribute\("aria-label",t\("createDeviceGroup"\)\);\s*\$\("#createGroupShortcut"\)\.title=t\("createDeviceGroup"\)/);
  assert.match(dashboard, /\$\("#createGroupShortcut"\)\.onclick=\(\)=>openGroupEditor\(\)/);
  assert.match(dashboard, /\{target:"#createGroupShortcut",title:"tourGroupTitle",text:"tourGroupLead"\}/);
  ...
  assert.match(dashboard, /function screensaverAllowed\(\)\{\s*return document\.body\.dataset\.activeView==="home"\s*&&!document\.body\.classList\.contains\("night"\)\s*&&!document\.querySelector\("dialog\[open\]"\)/);
  ...
  assert.match(dashboard, /if\(document\.body\.classList\.contains\("night"\)&&state\.screensaverOpen\)dismissScreensaver\(\)/);
  ...
  assert.match(dashboard, /#home \[data-widget="quick"\]\{height:76px;flex-basis:76px;order:2\}#home \.widget-rail\{order:1\}/);
